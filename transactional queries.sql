insert into featureofinterest
select nextval('featureofinterestid_seq'),'T',1,'campus',1,'campus',1,'campus',NULL,NULL,NULL;

select UpdateGeometrySRID('featureofinterest', 'geom', 4326);

update featureofinterest
set hibernatediscriminator = 'T';

insert into featurerelation
select a.id, b.id
from
(select featureofinterestid as id 
from featureofinterest 
where identifier = 'campus') a, 
(select featureofinterestid as id
from featureofinterest, (select featureofinterestid as id 
			  from featureofinterest 
			  where identifier = 'campus') a 
where featureofinterestid != a.id) b;

insert into offeringallowedfeaturetype (offeringid, featureofinteresttypeid)
select 1,1;

insert into offeringallowedobservationtype (offeringid, observationtypeid)
select 1,1;

insert into resulttemplate
select nextval('resulttemplateid_seq'), 1, 1, 1, 5, 'anything', 
'<swe:DataRecord xmlns:swe="http://www.opengis.net/swe/2.0" xmlns:xlink="http://www.w3.org/1999/xlink">
  <swe:field name="phenomenonTime">
    <swe:TimeRange xmlns:ns="http://www.opengis.net/swe/2.0" definition="http://www.opengis.net/def/property/OGC/0/PhenomenonTime">
      <ns:uom xlink:href="http://www.opengis.net/def/uom/ISO-8601/0/Gregorian"/>
    </swe:TimeRange>
  </swe:field>
  <swe:field name="resultTime">
    <swe:Time xmlns:ns="http://www.opengis.net/swe/2.0" definition="http://www.opengis.net/def/property/OGC/0/ResultTime">
      <ns:uom code="testunit1"/>
    </swe:Time>
  </swe:field>
  <swe:field name="observable_property_6">
    <swe:Quantity xmlns:ns="http://www.opengis.net/swe/2.0" definition="http://www.52north.org/test/observableProperty/6">
      <ns:uom code="test_unit_6"/>
    </swe:Quantity>
  </swe:field>
</swe:DataRecord>',
'<swe:TextEncoding xmlns:swe="http://www.opengis.net/swe/2.0" blockSeparator="#" tokenSeparator=","/>';

insert into validproceduretime
select nextval('validproceduretimeid_seq'), 1, 1, (select min(phenomenontimestart) from observation), NULL, '<sml:SensorML xmlns:sml="http://www.opengis.net/sensorML/1.0.1" xmlns:sos="http://www.opengis.net/sos/1.0" version="1.0.1">
  <sml:member>
    <sml:System xmlns:ogc="http://www.opengis.net/ogc" xmlns:ows="http://www.opengeospatial.net/ows" xmlns:om="http://www.opengis.net/om/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:gml="http://www.opengis.net/gml" xmlns:swe="http://www.opengis.net/swe/1.0.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
      <sml:identification>
        <sml:IdentifierList>
          <sml:identifier name="uniqueID">
            <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
              <sml:value>52NorthWS1</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="longName">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
              <sml:value>This is a WS2500 weather station setup at 52North, Münster in Germany.</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="shortName">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
              <sml:value>52North HWS 1</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="productName">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
              <sml:value>ELV Radio Weather Station WS 2500</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="modelNumber">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:modelNumber">
              <sml:value>53759</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="manufacturer">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:manufacturer">
              <sml:value>ELV Elektronik AG</sml:value>
            </sml:Term>
          </sml:identifier>
          <sml:identifier name="operator">
            <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:operator">
              <sml:value>52North, Münster, Germany.</sml:value>
            </sml:Term>
          </sml:identifier>
        </sml:IdentifierList>
      </sml:identification>
      <sml:classification>
        <sml:ClassifierList>
          <sml:classifier name="intendedApplication">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:application">
              <sml:value>weather</sml:value>
            </sml:Term>
          </sml:classifier>
          <sml:classifier name="sensorType">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
              <sml:value>thermometer</sml:value>
            </sml:Term>
          </sml:classifier>
          <sml:classifier name="sensorType">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
              <sml:value>rain gauge</sml:value>
            </sml:Term>
          </sml:classifier>
          <sml:classifier name="sensorType">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
              <sml:value>barometer</sml:value>
            </sml:Term>
          </sml:classifier>
          <sml:classifier name="sensorType">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
              <sml:value>anemometer</sml:value>
            </sml:Term>
          </sml:classifier>
          <sml:classifier name="sensorType">
            <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
              <sml:value>Illuminace Sensor</sml:value>
            </sml:Term>
          </sml:classifier>
        </sml:ClassifierList>
      </sml:classification>
      <sml:capabilities>
        <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
          <!--station is collecting data-->
          <swe:field name="status">
            <swe:Boolean>
              <swe:value>false</swe:value>
            </swe:Boolean>
          </swe:field>
          <swe:field name="mobile">
            <swe:Boolean>
              <swe:value>false</swe:value>
            </swe:Boolean>
          </swe:field>
          <swe:field name="measuringInterval">
            <swe:Quantity definition="urn:ogc:def:property:OGC:1.0:measuringInterval">
              <gml:description>The measuring interval of the weather station.</gml:description>
              <swe:uom code="min" xlink:href="urn:x-ogc:def:uom:OGC:min"/>
              <swe:value>3</swe:value>
            </swe:Quantity>
          </swe:field>
          <swe:field name="transmissionFrequency">
            <swe:Quantity definition="urn:ogc:def:property:OGC:1.0:transmissionFrequency">
              <gml:description>The transmission frequency of the weather station.</gml:description>
              <swe:uom code="MHz" xlink:href="urn:x-ogc:def:uom:OGC:MHz"/>
              <swe:value>433.92</swe:value>
            </swe:Quantity>
          </swe:field>
          <swe:field name="powerSupply">
            <swe:Text definition="urn:ogc:def:property:OGC:1.0:powerSupply">
              <gml:description>The power supply of the weather station.</gml:description>
              <swe:value>4 Baby cells 1.5V = 7,5 V / 500 mA</swe:value>
            </swe:Text>
          </swe:field>
        </swe:SimpleDataRecord>
      </sml:capabilities>
      <sml:contact>
        <sml:ResponsibleParty gml:id="contact">
          <sml:individualName>Jürrens, Eike Hinderk</sml:individualName>
          <sml:organizationName>52North, Münster, Germany.</sml:organizationName>
          <sml:contactInfo>
            <sml:phone>
              <sml:voice>+49 251 39 63 71 33</sml:voice>
              <sml:facsimile>+49 251 39 63 71 11</sml:facsimile>
            </sml:phone>
            <sml:address>
              <sml:deliveryPoint>Martin-Luther-King-Weg 24</sml:deliveryPoint>
              <sml:city>Münster</sml:city>
              <sml:administrativeArea>North Rhine-Westfalia</sml:administrativeArea>
              <sml:postalCode>49151</sml:postalCode>
              <sml:country>Germany</sml:country>
              <sml:electronicMailAddress>e.h.juerrens@52north.org</sml:electronicMailAddress>
            </sml:address>
          </sml:contactInfo>
        </sml:ResponsibleParty>
      </sml:contact>
      <sml:documentation xlink:role="urn:ogc:def:object:OGC:1.0:image">
        <sml:Document>
          <gml:description>photo of the weather station</gml:description>
          <sml:format>image/jpg</sml:format>
          <sml:onlineResource xlink:href="http://ifgi.uni-muenster.de/~e_juer01/WS2500.jpg"/>
        </sml:Document>
      </sml:documentation>
      <sml:history>
        <sml:EventList>
          <sml:member name="deployDate">
            <sml:Event>
              <sml:date>2007-06-01</sml:date>
              <gml:description>Event of deploying the weather station.</gml:description>
              <sml:contact xlink:href="#contact"/>
            </sml:Event>
          </sml:member>
        </sml:EventList>
      </sml:history>
      <sml:spatialReferenceFrame>
        <gml:EngineeringCRS gml:id="STATION_FRAME">
          <gml:srsName>Spatial Reference System of station</gml:srsName>
          <gml:usesCS xlink:href="urn:ogc:def:cs:OGC:1.0:Grid2dSquareCS"/>
          <gml:usesEngineeringDatum>
            <gml:EngineeringDatum gml:id="STATION_DATUM">
              <gml:datumName>Spatial Datum of station</gml:datumName>
              <gml:anchorPoint>Origin is the base of the station.
                                    Z is vertical.
                                    X and Y are orthogonal to Z.
                                    X is orthogonal to Y.
                                    Y is parallel to the North-Axis and points to North.
                                    X is orthogonal to the North-Axis and points to East.</gml:anchorPoint>
            </gml:EngineeringDatum>
          </gml:usesEngineeringDatum>
        </gml:EngineeringCRS>
      </sml:spatialReferenceFrame>
      <sml:position name="stationPosition">
        <swe:Position fixed="false" referenceFrame="urn:ogc:def:crs:EPSG::4326">
          <swe:location>
            <swe:Vector gml:id="STATION_LOCATION">
              <swe:coordinate name="northing">
                <swe:Quantity axisID="y">
                  <swe:uom code="deg" xlink:href="urn:x-ogc:def:uom:OGC:deg"/>
                  <swe:value>51.9348</swe:value>
                </swe:Quantity>
              </swe:coordinate>
              <swe:coordinate name="easting">
                <swe:Quantity axisID="x">
                  <swe:uom code="deg" xlink:href="urn:x-ogc:def:uom:OGC:deg"/>
                  <swe:value>7.6524</swe:value>
                </swe:Quantity>
              </swe:coordinate>
              <swe:coordinate name="altitude">
                <swe:Quantity axisID="z">
                  <!--200-->
                  <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                  <swe:value>200</swe:value>
                </swe:Quantity>
              </swe:coordinate>
            </swe:Vector>
          </swe:location>
        </swe:Position>
      </sml:position>
      <sml:inputs>
        <sml:InputList>
          <sml:input name="precipitation">
            <swe:ObservableProperty definition="Precipitation"/>
          </sml:input>
          <sml:input name="atmosphericTemperature">
            <swe:ObservableProperty definition="AirTemperature"/>
          </sml:input>
          <sml:input name="atmosphericPressure">
            <swe:ObservableProperty definition="AtmosphericPressure"/>
          </sml:input>
          <sml:input name="wind">
            <swe:ObservableProperty definition="Wind"/>
          </sml:input>
          <sml:input name="illuminace">
            <swe:ObservableProperty definition="Illuminace"/>
          </sml:input>
          <sml:input name="humidity">
            <swe:ObservableProperty definition="RelativeHumidity"/>
          </sml:input>
        </sml:InputList>
      </sml:inputs>
      <sml:outputs>
        <sml:OutputList>
          <sml:output name="precipitation">
            <swe:Quantity definition="Precipitation1Hour">
              <swe:uom code="mm" xlink:href="urn:x-ogc:def:uom:OGC:mm"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="windDirection">
            <swe:Quantity definition="WindDirection">
              <swe:uom code="deg" xlink:href="urn:x-ogc:def:uom:OGC:deg"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="windSpeed">
            <swe:Quantity definition="WindSpeed">
              <swe:uom code="m/s" xlink:href="urn:x-ogc:def:uom:OGC:m_s"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="barometricPressure">
            <swe:Quantity definition="BarometricPressure">
              <swe:uom code="hPa" xlink:href="urn:x-ogc:def:uom:OGC:hPa"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="temperature">
            <swe:Quantity definition="Temperature">
              <swe:uom code="Cel" xlink:href="urn:x-ogc:def:uom:OGC:degC"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="illuminace">
            <swe:Quantity definition="Illuminace">
              <swe:uom code="lx" xlink:href="urn:x-ogc:def:uom:OGC:lx"/>
            </swe:Quantity>
          </sml:output>
          <sml:output name="relativeHumidity">
            <swe:Quantity definition="RelativeHumidity">
              <swe:uom code="%" xlink:href="urn:x-ogc:def:uom:OGC:percent"/>
            </swe:Quantity>
          </sml:output>
        </sml:OutputList>
      </sml:outputs>
      <sml:components>
        <sml:ComponentList>
          <sml:component name="rain-gauge">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:rain-gauge-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Rain-Gauge at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Rain-Gauge 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2000R-1 precipitation sensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>Precipitation</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="rainGaugePosition">
                <swe:Position>
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="precipitation">
                    <swe:ObservableProperty definition="Precipitation"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="precipitation">
                    <swe:Quantity definition="Precipitation1Hour">
                      <swe:uom code="mm" xlink:href="urn:x-ogc:def:uom:OGC:mm"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
          <sml:component name="radiometer">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:radiometer-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="longName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Radiometer at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="shortName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Radiometer 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2500H illuminace sensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>radiometer</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="radiometerPosition">
                <swe:Position referenceFrame="urn:ogc:def:crs:EPSG::4326">
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="illuminace">
                    <swe:ObservableProperty definition="Illuminace"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="illuminace">
                    <swe:Quantity definition="Illuminace">
                      <swe:uom code="lx" xlink:href="urn:x-ogc:def:uom:OGC:lx"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
          <sml:component name="hygrometer">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:hygrometer-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="longName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Hygrometer at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="shortName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Hygrometer 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2500H humidity sensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>hygrometer</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="hygrometerPosition">
                <swe:Position referenceFrame="urn:ogc:def:crs:EPSG::4326">
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="humidity">
                    <swe:ObservableProperty definition="Humidity"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="relativeHumidity">
                    <swe:Quantity definition="RelativeHumidity">
                      <swe:uom code="%" xlink:href="urn:x-ogc:def:uom:OGC:percent"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
          <sml:component name="aenometer">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:aenometer-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Aenometer at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Aenometer 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2000W windsensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <!--FIXME kann es 2 classifier fÃƒÂ¼r eine Component geben?-->
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>windSpeed</sml:value>
                    </sml:Term>
                  </sml:classifier>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>windDirection</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="aenometerPosition">
                <swe:Position>
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="wind">
                    <swe:ObservableProperty definition="Wind"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="windSpeed">
                    <swe:Quantity definition="WindSpeed">
                      <gml:metaDataProperty>
                        <sos:offering>
                          <sos:id>WIND_SPEED</sos:id>
                          <sos:name>The speed of the measured wind.</sos:name>
                        </sos:offering>
                      </gml:metaDataProperty>
                      <swe:uom code="m/s" xlink:href="urn:x-ogc:def:uom:OGC:m_s"/>
                    </swe:Quantity>
                  </sml:output>
                  <sml:output name="windDirection">
                    <swe:Quantity definition="WindDirection">
                      <swe:uom code="deg" xlink:href="urn:x-ogc:def:uom:OGC:deg"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
          <sml:component name="barmometer">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:barometer-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="longName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Barometer at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="shortName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Barometer 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2000B pressure sensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>barometer</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="thermometerPosition">
                <swe:Position referenceFrame="urn:ogc:def:crs:EPSG::4326">
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="atmosphericPressure">
                    <swe:ObservableProperty definition="AtmospericPressure"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="barometricPressure">
                    <swe:Quantity definition="BarometricPressure">
                      <swe:uom code="hPa" xlink:href="urn:x-ogc:def:uom:OGC:hPa"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
          <sml:component name="thermometer">
            <sml:System>
              <sml:identification>
                <sml:IdentifierList>
                  <sml:identifier>
                    <sml:Term definition="urn:ogc:def:identifier:OGC:uniqueID">
                      <sml:value>urn:ogc:object:feature:Sensor:OSIRIS-HWS:thermometer-3d3b239f-7696-4864-9d07-15447eae2b93</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="longName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:longName">
                      <sml:value>OSIRIS Thermometer at weather station 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="shortName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:shortName">
                      <sml:value>OSIRIS Thermometer 52N HWS 1</sml:value>
                    </sml:Term>
                  </sml:identifier>
                  <sml:identifier name="productName">
                    <sml:Term definition="urn:ogc:def:identifier:OGC:1.0:productName">
                      <sml:value>S2001IA temperature sensor</sml:value>
                    </sml:Term>
                  </sml:identifier>
                </sml:IdentifierList>
              </sml:identification>
              <sml:classification>
                <sml:ClassifierList>
                  <sml:classifier name="sensorType">
                    <sml:Term definition="urn:ogc:def:classifier:OGC:1.0:sensorType">
                      <sml:value>thermometer</sml:value>
                    </sml:Term>
                  </sml:classifier>
                </sml:ClassifierList>
              </sml:classification>
              <sml:capabilities>
                <swe:SimpleDataRecord definition="urn:ogc:def:property:status">
                  <swe:field name="status">
                    <swe:Text definition="urn:ogc:def:property:OGC:1.0:status">
                      <gml:description>The operating status of the sensor.</gml:description>
                      <swe:value>active</swe:value>
                    </swe:Text>
                  </swe:field>
                </swe:SimpleDataRecord>
              </sml:capabilities>
              <sml:position name="thermometerPosition">
                <swe:Position referenceFrame="urn:ogc:def:crs:EPSG::4326">
                  <swe:location>
                    <swe:Vector>
                      <swe:coordinate name="easting">
                        <swe:Quantity axisID="x">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="northing">
                        <swe:Quantity axisID="y">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                      <swe:coordinate name="altitude">
                        <swe:Quantity axisID="z">
                          <swe:uom code="m" xlink:href="urn:x-ogc:def:uom:OGC:m"/>
                          <swe:value>0</swe:value>
                        </swe:Quantity>
                      </swe:coordinate>
                    </swe:Vector>
                  </swe:location>
                </swe:Position>
              </sml:position>
              <sml:inputs>
                <sml:InputList>
                  <sml:input name="atmosphericTemperature">
                    <swe:ObservableProperty definition="AirTemperature"/>
                  </sml:input>
                </sml:InputList>
              </sml:inputs>
              <sml:outputs>
                <sml:OutputList>
                  <sml:output name="temperature">
                    <swe:Quantity definition="Temperature">
                      <swe:uom code="Cel" xlink:href="urn:x-ogc:def:uom:OGC:degC"/>
                    </swe:Quantity>
                  </sml:output>
                </sml:OutputList>
              </sml:outputs>
            </sml:System>
          </sml:component>
        </sml:ComponentList>
      </sml:components>
    </sml:System>
  </sml:member>
</sml:SensorML>'
